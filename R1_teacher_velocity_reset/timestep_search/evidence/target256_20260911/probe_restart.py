"""256배 원래 실패 상태에서 restart240 설정을 검산한다."""
...
source=p.parent/'ten_second_v1/trials/sub001/failure_prefix.npz'
with np.load(source,allow_pickle=False) as z:trace=dict(z)
with np.load(p/'wind.npz',allow_pickle=False) as z:wind=z['wind_m_s'][80]
...
initial=s.state(displacement=trace['u_m'][-1],velocity=trace['v_m_s'][-1],time_s=trace['time_s'][-1]);force=m.aerodynamic_force_displacement(trace['u_m'][0],trace['v_m_s'][0],wind)['force_n']
...
report={'input_time_s':initial.time_s,'target_time_s':initial.time_s+1/60,'frame':80,'environment':environment,'policy':asdict(base),'input_sha256':hashlib.sha256(source.read_bytes()).hexdigest(),'runtime_manifest_sha256':hashlib.sha256((p/'runtime_manifest.json').read_bytes()).hexdigest(),'trials':[],'training_eligible':False,'r1_complete':False}
