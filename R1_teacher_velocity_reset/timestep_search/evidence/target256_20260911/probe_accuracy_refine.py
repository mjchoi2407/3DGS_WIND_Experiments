"""256배 정확도 진단을 위한 동일 시작 상태·고정 공력의 세분 참조 계산."""
...
report={'input_time_s':initial.time_s,'target_time_s':initial.time_s+1/60,'frame':195,'environment':environment,'policy':asdict(base),'input_sha256':hashlib.sha256(source.read_bytes()).hexdigest(),'runtime_manifest_sha256':hashlib.sha256((p/'runtime_manifest.json').read_bytes()).hexdigest(),'trials':[],'training_eligible':False,'r1_complete':False}
trial_restart=240
for sub in [32]:
