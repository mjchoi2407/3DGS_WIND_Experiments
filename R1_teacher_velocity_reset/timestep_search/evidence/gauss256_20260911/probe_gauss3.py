...
report={'input_sha256':hashlib.sha256(source.read_bytes()).hexdigest(),'source_sha256':hashlib.sha256((out/'source.zip').read_bytes()).hexdigest(),'environment':environment,'input_time_s':state.time_s,'dt_s':h,'stages':3,'substeps':1,'multiplier':256,'training_eligible':False}
start=time.perf_counter();print('Gauss256 계산 시작',flush=True)
try:end,d=gauss_step(s,state,force,h,stages=3)
...
ld=np.longdouble;r=np.sqrt(ld(15))
coef=np.array([[ld(5)/36,ld(2)/9-r/15,ld(5)/36-r/30],
 [ld(5)/36+r/24,ld(2)/9,ld(5)/36-r/24],
 [ld(5)/36+r/30,ld(2)/9+r/15,ld(5)/36]])
weights=np.array([ld(5)/18,ld(4)/9,ld(5)/18])
