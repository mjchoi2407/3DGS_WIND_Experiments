...
refs={}
for n,folder in [(32,'20260911_gauss256_followup_reference_v1'),(64,'20260911_gauss256_followup_refine_v1')]:
...
LD=np.longdouble
refctrl=[]
for a,b in zip(refs[64][:-1],refs[64][1:]):refctrl.append({'u':np.stack([a['u'],a['u']+h/128*a['v'],b['u']]),'v':np.stack([a['v'],b['v']])})
peaks={k:max(norm(x[k]) for x in refs[64]) for k in ['u','v']}
results={}
for stages in [4,6]:
 ...
 for j,fine in enumerate(refctrl):
  for k in peaks:
   coarse=restrict(controls[k],LD(j)/64,LD(j+1)/64)
...
for j,fine in enumerate(refctrl):
 a,b=refs[32][j//2:j//2+2]
 ctrls={'u':np.stack([a['u'],a['u']+h/64*a['v'],b['u']]),'v':np.stack([a['v'],b['v']])}
 for k in peaks:
  segment=restrict(ctrls[k],LD(j%2)/2,LD(j%2+1)/2);delta=segment-fine[k]
  for q in range(4):upper[k]=max(upper[k],max(norm(x) for x in restrict(delta,LD(q)/4,LD(q+1)/4)))
results['reference32_to64']={'relative_upper':{k:upper[k]/peaks[k] for k in peaks}}
report={'scope':'주어진 한 구간의 Gauss collocation 곡선 대64분할 Newmark 수치 곡선. Bernstein convex-hull L2 상한+명시적 부동소수 여유. 엄밀한 interval arithmetic 또는 독립 ODE 참해 인증 아님.', 'results':results,'training_eligible':False}
