...
def compare(a,b):
 ...
 for i in range(N):
  for k in initial:
   ...
   # 같은 Bernstein control 질량 내적을 작은 Gram 행렬로 재사용한다.
   weighted=np.stack([m.mass@x for x in delta]);flat=delta.reshape(degree+1,-1)
   gram=flat@weighted.reshape(degree+1,-1).T
   for q in range(16):
    transform=restrict(np.eye(degree+1,dtype=LD),LD(q)/16,LD(q+1)/16)
    norms=np.sqrt(np.maximum(np.einsum('ij,jk,ik->i',transform,gram,transform),0))
    upper[k]=max(upper[k],float(norms.max()));lower[k]=max(lower[k],float(norms[0]),float(norms[-1]))
