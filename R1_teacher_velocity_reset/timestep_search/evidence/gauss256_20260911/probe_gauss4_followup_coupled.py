...
state=load_checkpoint(source,s);force=m.aerodynamic_force_displacement(state.displacement_m,state.velocity_m_s,wind)['force_n'];h=1/60
report={'input_sha256':hashlib.sha256(source.read_bytes()).hexdigest(),'source_sha256':hashlib.sha256((out/'source.zip').read_bytes()).hexdigest(),'environment':environment,'input_time_s':state.time_s,'dt_s':h,'stages':4,'substeps':1,'multiplier':256,'training_eligible':False}
start=time.perf_counter();print('Gauss256 계산 시작',flush=True)
try:end,d=gauss_step(s,state,force,h,stages=4,preconditioner_kind='coupled')
...
ld=np.longdouble;nodes,qweights=np.polynomial.legendre.leggauss(4)
nodes=(nodes.astype(ld)+1)/2;weights=qweights.astype(ld)/2
coef=np.zeros((4,4),dtype=ld)
for i,c in enumerate(nodes):
 for j in range(4):
  values=np.ones(4,dtype=ld)
  for k in range(4):
...
ref=p.parent/'20260911_gauss256_followup_reference_v1/sub32_step31.npz'
