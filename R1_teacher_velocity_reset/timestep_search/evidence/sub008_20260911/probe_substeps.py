"""동일 실패 직전 상태에서4분할1step과8분할2step을 비교한다."""
...
p=Path('experiments/artifacts/runs/teacher_timestep_search/20260911_precision_segments_v3');verify_runtime(p)
...
report={'input_time_s':initial.time_s,'target_time_s':initial.time_s+1/240,'frame':195,'environment':environment,'policy':asdict(base),'input_sha256':hashlib.sha256((p/'trials/sub004/failure_prefix.npz').read_bytes()).hexdigest(),'runtime_manifest_sha256':hashlib.sha256((p/'runtime_manifest.json').read_bytes()).hexdigest(),'trials':[],'training_eligible':False,'r1_complete':False}
for sub in [4,8]:
 state=initial;dt=1/(60*sub);trial={'substeps':sub,'dt_s':dt,'rows':[],'status':'passed'}
 for index in range(sub//4):
