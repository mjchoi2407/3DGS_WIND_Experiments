"""8분할로 통과한 상태에서 다음 한 frame의8개 step을 검산한다."""
...
checkpoint=p.parent/'20260911_sub008_probe_v1/sub8_step1.npz'
...
report={'input_time_s':initial.time_s,'target_time_s':initial.time_s+1/60,'frame':196,'environment':environment,'policy':asdict(base),'input_sha256':hashlib.sha256(checkpoint.read_bytes()).hexdigest(),'runtime_manifest_sha256':hashlib.sha256((p/'runtime_manifest.json').read_bytes()).hexdigest(),'trials':[],'training_eligible':False,'r1_complete':False}
for sub in [8]:
