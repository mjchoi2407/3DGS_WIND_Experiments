...
source=p.parent/'20260911_acceleration4_segments_v1/trials/sub064/frames/000.npz'
with np.load(p/'wind.npz',allow_pickle=False) as z:wind=z['wind_m_s'][1]
s,environment=make_shell_stepper(32,device='cuda:0',backend='precision_hvp_graph');s.policy=dynamics.ShellSolvePolicy(**read(p/'plan.json')['policy']);s.policy=replace(s.policy,linear_preconditioner='rest');base=s.policy;m=s.model;bounder=P3ShellBounds(m.reference)
with np.load(source,allow_pickle=False) as z:trace=decode_trace(dict(z),'hi_lo_v1')
initial=s.state(displacement=trace['u_m'][-1],velocity=trace['v_m_s'][-1],time_s=float(trace['time_s'][-1]))
mode=sys.argv[2]
s.policy=replace(s.policy,linear_preconditioner='rest' if mode=='adaptive' else mode);base=s.policy
if mode=='adaptive':
 from wind3dgs.teacher.p3_shell_adaptive_preconditioner import AdaptivePreconditionerStepper
 s=AdaptivePreconditionerStepper(s)
if mode=='current':
...
report={'input_time_s':initial.time_s,'target_time_s':initial.time_s+1/960,'frame':1,'environment':environment,'policy':asdict(base),'input_sha256':hashlib.sha256(source.read_bytes()).hexdigest(),'runtime_manifest_sha256':hashlib.sha256((p/'runtime_manifest.json').read_bytes()).hexdigest(),'trials':[],'training_eligible':False,'r1_complete':False}
trial_restart=240
for sub in [64]:
 ...
 for index in range(sub//16):
  ...
  row={'index':index,'solve_s':time.perf_counter()-start,'attempts':d['attempts'],'hvp_calls':d['hvp_calls'],'adaptive':d.get('adaptive_preconditioner')}
