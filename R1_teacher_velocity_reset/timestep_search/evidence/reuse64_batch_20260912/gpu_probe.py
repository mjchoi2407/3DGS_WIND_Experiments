...
base=Path('experiments/artifacts/runs/teacher_timestep_search');old=base/'20260912_three_scenes_10s_v2';plan=verify(base/'20260912_three_scenes_10s_v4');assert plan['preconditioner_rebuild_every']==64
out=base/'20260912_three_scene_reuse64_check_v1';out.mkdir(exist_ok=False);results={}
for shape in ['reference_rectangle','triangular_flag','handkerchief']:
 ...
 frame=364 if shape=='reference_rectangle' else 1
 def trace(index):
  with np.load(source/shape/'frames'/f'{index:03d}.npz') as z:return decode_trace(dict(z),'hi_lo_v1')
 initial=trace(frame-1)
 expected=trace(frame) if shape!='reference_rectangle' else None
 with np.load(old/'inputs/wind.npz') as z:current_wind=z['wind_m_s'][frame]
 ...
 adaptive=AdaptivePreconditionerStepper(raw,switch_iterations=32,rebuild_every=plan['preconditioner_rebuild_every']);bounds=P3ShellBounds(model)
 force=raw.model.aerodynamic_force_displacement(state.displacement_m,state.velocity_m_s,current_wind)['force_n']
 ...
 for step in range(64):
  ...
  if expected is not None:
   du=max(du,float(np.max(abs(end.displacement_m-expected['u_m'][step+1]))));dv=max(dv,float(np.max(abs(end.velocity_m_s-expected['v_m_s'][step+1]))))
  ratio=max(ratio,check['force_ratio']);state=end
 assert du<=1e-16 and dv<=1e-12,(du,dv)
 results[shape]={'frame':frame,'steps':64,'position_difference_m':du if expected is not None else None,'velocity_difference_m_s':dv if expected is not None else None,'rebuild_every':plan['preconditioner_rebuild_every'],'max_force_ratio':ratio,'passed':True}
