...
main=Path('experiments/artifacts/runs/teacher_timestep_search/20260911_adaptive4_segments_v2');plan=read(main/'plan.json');frame=127
r=read(trial_dir(main,64)/'report.json');assert r['completed_frames']>=frame
trace=load_frame(trial_dir(main,64),frame-1,r)
raw,env=make_shell_stepper(32,device='cuda:0',backend='precision_hvp_graph');raw.policy=ShellSolvePolicy(**plan['policy']);official_policy=raw.policy
...
with np.load(main/'wind.npz',allow_pickle=False) as z:winds=z['wind_m_s'][frame:frame+4].copy()
...
report={'input_time_s':initial.time_s,'frame':frame,'dt_s':1/(60*64),'steps':256,'frame_count':4,'policy':asdict(raw.policy),'environment':env,'gpu_shared_with_main':False,'timing_is_controlled_benchmark':False,'trials':[], 'main_modified':False,'training_eligible':False,'r1_complete':False,'source_frame_sha256':hashlib.sha256((trial_dir(main,64)/'frames'/f'{frame-1:03d}.npz').read_bytes()).hexdigest()}
paths={}
def dump(): (out/'report.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n')
for mode in ['ew','margin30','margin30_cap4']:
 ...
 for i in range(256):
  ...
  if i%32==0: print(f'{mode}: {i+1}/256 단계, 계산 {trial["solve_s"]:.2f}초',flush=True);dump()
 if len(rows)==256 and all(x['audit_passed'] for x in rows):trial['status']='passed'
...
comparisons={};reference=paths['ew']
if len(reference[0])==257:
 for mode,(U,V) in paths.items():
  if len(U)!=257:continue
  du=[a-b for a,b in zip(U,reference[0])];dv=[a-b for a,b in zip(V,reference[1])]
  upper_u=max([norm(x) for x in du]+[norm(du[i]+.5*report['dt_s']*dv[i]) for i in range(256)])
