...
for step in range(2,10):
 ...
 audits={}
 for mode,model,dtype in [('extended',s.model,ld),('float64_export',original_model,float)]:
...
force_cost={}
for name,model in [('float64_gpu',original_model),('extended_cpu',s.model)]:
...
report={'environment':env,'longdouble_mantissa_bits_including_hidden':int(np.finfo(ld).nmant+1),'policy':s.policy.__dict__,'scope':'상태·힘은 CPU longdouble, GMRES/HVP는 기존 float64 GPU 근사 접선. 저장 형식 미채택 진단.','rows':rows,'elapsed_with_audit_s':time.perf_counter()-start,'force_evaluation_mean_s':force_cost,'force_eval_counts':s.model.calls,'source_sha256':manifest,'training_eligible':False,'r1_complete':False,'input_npz_sha256':hashlib.sha256((p/'sub016.npz').read_bytes()).hexdigest()}
