...
s,env=make_shell_stepper(32,device='cuda:0',backend='hvp_graph');original_model=s.model;s.model=HighPrecisionModel(original_model);s.mass_factor=FloatFactor(s.mass_factor)

p=Path('experiments/artifacts/runs/teacher_timestep_search/20260911_highprecision_state_v2');z=np.load(p/'states_hi_lo.npz');U=z['u_hi'].astype(ld)+z['u_lo'].astype(ld);V=z['v_hi'].astype(ld)+z['v_lo'].astype(ld)
...
assert r['u_exact'] and r['v_exact'] and r['time_exact'];print(json.dumps(r),flush=True)
