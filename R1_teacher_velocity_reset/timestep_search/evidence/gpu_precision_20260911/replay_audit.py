...
base=p.parent/'20260911_incremental_newton_v1';old=np.load(base/'sub016.npz');wind=np.load(p.parent/'ten_second_v1/wind.npz')['wind_m_s'];force=original_model.aerodynamic_force_displacement(old['u_m'][-3],old['v_m_s'][-3],wind[160])['force_n'];dt=1/(60*16);s.preconditioners[dt]=FloatFactor(splu(s.M+(.25*dt*dt)*s.K));initial=state128(U[4],V[4],z['time_s'][4]);result,diag=s.step(initial,force,dt)
r={'replayed_substep':6,'u_exact':bool(np.array_equal(result.displacement_m,U[5])),'v_exact':bool(np.array_equal(result.velocity_m_s,V[5])),'time_exact':bool(result.time_s==float(z['time_s'][5])),'max_u_difference_m':float(np.max(abs(result.displacement_m-U[5]))),'max_v_difference_m_s':float(np.max(abs(result.velocity_m_s-V[5])))}

audits={}
for label,u1,v1 in [('replayed',result.displacement_m,result.velocity_m_s),('saved',U[5],V[5])]:
 u0=initial.displacement_m;v0=initial.velocity_m_s
 e0=force_function(s.model.reference,u0);e1=force_function(s.model.reference,u1)
 a0=np.zeros_like(u0);a0[s.model.free]=s.mass_factor.solve((force+e0['force_n'])[s.model.free])
 a1=2*(v1-v0)/dt-a0;ma=s.model.mass@a1;res=ma-e1['force_n']-force
 limit=s.policy.force_atol_n+s.policy.force_rtol*max(np.linalg.norm(x[s.model.free]) for x in [ma,force,e1['force_n']])
 audits[label]={'residual_ratio':float(np.linalg.norm(res[s.model.free])/limit),'position_update_error_m':float(abs(u0+dt*v0+dt*dt/4*(a0+a1)-u1).max())}
r['independent_audits']=audits
