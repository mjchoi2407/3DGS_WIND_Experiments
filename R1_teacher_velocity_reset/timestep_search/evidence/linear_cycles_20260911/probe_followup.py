"""한도12로 통과한 상태에서 다음 한 frame의4개 interval을 검산한다."""
...
checkpoint=p.parent/'20260911_linear_cycles_probe_v1/cycles12_state.npz'
state=load_checkpoint(checkpoint,s)
with np.load(p/'wind.npz') as z:wind=z['wind_m_s'][162]
force=m.aerodynamic_force_displacement(state.displacement_m,state.velocity_m_s,wind)['force_n'];dt=1/240
...
report={'input_time_s':state.time_s,'frame':162,'substep':0,'dt_s':dt,'environment':environment,'base_policy':asdict(base),'input_sha256':hashlib.sha256(checkpoint.read_bytes()).hexdigest(),'runtime_manifest_sha256':hashlib.sha256((p/'runtime_manifest.json').read_bytes()).hexdigest(),'trials':[],'training_eligible':False,'r1_complete':False}
for index in range(4):
 ...
 try:
  ...
  save_checkpoint(out/f'substep{index}_state.npz',end)
 r['substep']=index;report['trials'].append(r);(out/'report.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n');print(json.dumps(r),flush=True)
 if r['status']!='passed' or not r['audit_passed']:break
 state=end
