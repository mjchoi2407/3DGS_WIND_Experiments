...
p=argparse.ArgumentParser();p.add_argument('--skip-audit',action='store_true');p.add_argument('--parent',type=Path,required=True);p.add_argument('--output',type=Path,required=True);a=p.parse_args();a.output.mkdir(parents=True,exist_ok=False)
...
for name,Model,Stepper in [('기준',P3ShellWarp,P3ShellWarpStepper),('HVP 전용+graph',P3ShellWarpFast,P3ShellWarpFastStepper),('CuPy 조기 종료+분석 재사용+graph',P3ShellWarpFast,P3ShellCuPyStepper)]:
 t=time.perf_counter();m=Model(P3Shell(32),device='cuda:0',**({'capture':True} if Model is P3ShellWarpFast else {}));s=Stepper(m);setup=time.perf_counter()-t
 print(name,'초기화 완료',setup,flush=True)
 state=s.state(displacement=trace['u_m'][start],velocity=trace['v_m_s'][start],time_s=float(trace['time_s'][start]));dt=1/(60*256)
 t=time.perf_counter();s.step(state,trace['held_force_n'],dt);warmup=time.perf_counter()-t
 print(name,'워밍업 완료',warmup,flush=True)
 samples=[]
 for _ in range(3):
  t=time.perf_counter();out,diagnostic=s.step(state,trace['held_force_n'],dt);samples.append(time.perf_counter()-t)
...
report['audit']=verify_frame(m,trace,metadata,frame=39,substeps=256,policy=ShellSolvePolicy(),cpu_reference=m.reference,wind_scale=4.)
