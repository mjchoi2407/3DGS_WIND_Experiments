...
def main():
    ...
    grid = fig.add_gridspec(2, 3, left=.04, right=.9, top=.87, bottom=.09, hspace=.28, wspace=.18)
    axes = []
    ...
    fig.colorbar(plt.cm.ScalarMappable(norm=norm, cmap=cmap), cax=fig.add_axes([.935, .23, .018, .51]),
    ...
    for extension in ("png", "pdf"):
        fig.savefig(args.output / ("natural_shapes." + extension), dpi=170, bbox_inches="tight")
    plt.close(fig)
