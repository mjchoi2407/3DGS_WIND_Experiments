...
> **2026-09-22 보존 상태:** 최종 판정·config·manifest·입력·source snapshot을 유지하고 전체 frame 배열은
> report-only로 정리했다. `compare_space16_32_s256_reset66`의 대표 66·67·89 frame은 n16/n32 양쪽에
> 남겼다. 아래 이어하기 명령은 당시 실행 계약이며, 현재 compact 결과에 그대로 실행하면 누락 frame hash에서
> 중단할 수 있다. 전체 궤적이 필요하면 기존 스크립트와 보존 입력으로 새 output에서 처음부터 재계산한다.
...
당시 이어하기 계약은 원래 자료가 바뀌면 hash 확인에서 중단하도록 했다. 2026-09-22 이후에는 위 compact
보존 상태를 기준으로 하며, 전체 ignored 원본이 필요하면 새 output으로 재계산한다.
