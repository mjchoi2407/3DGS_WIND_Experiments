...
ROOT=pathlib.Path('experiments/artifacts/datasets/mesh_candidates_20260912')
OUT=pathlib.Path('experiments/mesh_candidate_audit/evidence');OUT.mkdir(exist_ok=True,parents=True)
def topology(v,f):
...
for d in sorted(ROOT.iterdir()):
 ...
 for name,m in scene.geometry.items():
  t=meshstats(m);t['name']=name;t['material']=getattr(m.visual.material,'name',None);uv=getattr(m.visual,'uv',None)
